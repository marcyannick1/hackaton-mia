  ...
  return (
    <div className="space-y-5 h-full overflow-y-auto pb-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 px-5 py-4 flex items-center gap-4">
        <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center text-base font-semibold text-blue-700 flex-shrink-0">
          {f.initiales}
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-semibold text-slate-800">{f.raisonSociale}</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            SIRET {f.siret} · {f.adresse} · Fournisseur actif depuis {f.depuis}
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Docs uploadés", value: f.metrics.docsUploades, sub: f.metrics.docsValides + " validés · " + f.metrics.docsAlertes + " alerte" },
          { label: "Statut URSSAF", value: <Badge statut={f.urssafStatut} />, sub: "expire le " + f.urssafExpire },
        ].map(m => (
              ...
              <div key={doc.nom} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-md bg-slate-100 flex items-center justify-center flex-shrink-0">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" className="text-slate-500">
          ...
          <div className="space-y-1">
            {f.factures.map(fac => (
              <div key={fac.ref} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                <div>
                  <p className="text-xs font-semibold text-blue-600">{fac.ref}</p>
                  <p className="text-xs text-slate-400">{fac.date}</p>
                </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 px-5 py-4">
          ...
          <div className="space-y-1">
            {f.alertes.map((alerte, i) => {
              const dotClass = alerte.niveau === "error"
                ? "w-2 h-2 rounded-full flex-shrink-0 mt-1 bg-red-500"
                : alerte.niveau === "warn"
                ? "w-2 h-2 rounded-full flex-shrink-0 mt-1 bg-amber-400"
                : "w-2 h-2 rounded-full flex-shrink-0 mt-1 bg-emerald-500";
              return (
      ...
      {showEdit && (
        <EditFournisseurModal fournisseur={f} onClose={() => setShowEdit(false)} />
      )}
    </div>
  );
}