  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 h-full flex flex-col">
      <div className="px-4 py-4 border-b border-slate-100">
        ...
        <p className="text-xs text-slate-400 mt-0.5">{fournisseurs.length} enregistrés</p>
      </div>
      <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
        {fournisseurs.map(f => {
          const isSelected = selected?.id === f.id;
          const btnClass = isSelected
            ? "w-full text-left px-4 py-3.5 transition-all hover:bg-slate-50 bg-blue-50 border-l-2 border-l-blue-500"
            : "w-full text-left px-4 py-3.5 transition-all hover:bg-slate-50";
          return (
            <button key={f.id} onClick={() => onSelect(f)} className={btnClass}>
