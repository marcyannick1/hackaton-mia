  ...
  return (
    <div className="flex items-center gap-4 px-4 py-3 bg-slate-50 rounded-lg border border-slate-200">
      <div className="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center flex-shrink-0">
        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" className="text-slate-500"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
      </div>
      ...
      <button onClick={() => onRemove(file.name)} className="text-slate-300 hover:text-red-400 transition-colors flex-shrink-0">
    ...
    <div className="space-y-4">
      <div onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-slate-700">{dragging ? "Deposez vos fichiers ici" : "Glissez-deposez vos fichiers"}</p>
            <p className="text-xs text-slate-400 mt-1">PDF, PNG, JPEG · Plusieurs fichiers acceptes</p>
          </div>
