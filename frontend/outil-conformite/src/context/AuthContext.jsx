...
export function AuthProvider({ children }) {
  ...
  const [user, setUser] = useState(() => {
    try { const s = localStorage.getItem("conf_user"); return s ? JSON.parse(s) : null; } catch { return null; }
  });
  ...
  const login = (email, password) => {
    ...
    if (found) {
      ...
      setUser(safe); localStorage.setItem("conf_user", JSON.stringify(safe)); setError(""); return true;
    }
  ...
  const logout = () => { setUser(null); localStorage.removeItem("conf_user"); };
