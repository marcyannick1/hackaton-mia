    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          {[{ name: "nom", label: "Nom complet", type: "text" }, { name: "email", label: "Email", type: "email" }].map(f => (
            <div key={f.name}>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">{f.label}</label>
              <input type={f.type} name={f.name} value={form[f.name]} onChange={handleChange} required className="w-full h-10 px-3.5 text-sm bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all" />
            </div>
          ))}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Role</label>
            <select name="role" value={form.role} onChange={handleChange} className="w-full h-10 px-3.5 text-sm bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
              <option>Operateur</option><option>Administrateur</option>
            </select>
          </div>
          {[{ name: "password", label: "Mot de passe" }, { name: "confirmPassword", label: "Confirmer" }].map(f => (
            <div key={f.name}>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">{f.label}</label>
              <input type="password" name={f.name} value={form[f.name]} onChange={handleChange} required className="w-full h-10 px-3.5 text-sm bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all" />
            </div>
          ))}
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 text-center">
          <p className="text-xs text-slate-400">Deja un compte ?{" "}<Link to="/login" className="text-emerald-600 font-medium">Se connecter</Link></p>
        </div>
      </div>
    </div>
  );
